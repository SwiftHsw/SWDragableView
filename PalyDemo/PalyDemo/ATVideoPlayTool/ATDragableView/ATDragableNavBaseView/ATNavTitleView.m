...
static float const ATPaddingNormal = 10.f;


@interface ATNavTitleView ()
@property(nonatomic,readwrite) UILabel *titleLabel;
@property(nonatomic,readwrite)UIView *splitView;
@end

@implementation ATNavTitleView
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if(self){
        ...
        _contentOffsetY = ATStatusBarHeight / 2.0 ;
        [self setupUI];
    }
    return self;
}
...
- (void)setupUI{
    ...
    for(UIButton *btn in self.leftBtns){
        ...
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if(lastBtn == nil){
                make.left.mas_equalTo(self).mas_offset(ATPaddingNormal);
            }else{
    ...
    [self.titleLabel mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self).mas_offset(2 * ATPaddingNormal);
        make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
        make.width.mas_equalTo(self).mas_offset(-4 * ATPaddingNormal).priority(998);
    ...
    for(NSInteger i = self.rightBtns.count - 1 ; i >=0 ; i--){
        ...
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(btn.size); make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
            if(lastBtn == nil){
                make.right.mas_equalTo(self).mas_offset(-ATPaddingNormal);
            }else{
...
- (UIView *)splitView{
    if(!_splitView){
        _splitView = ({
            ...
            view.backgroundColor = ATColor(244, 245, 246, 1.0);;
            view ;
        });
    }
    return _splitView;
